...
trunkComponentsCGsToTrunkCG = trunkComponentCGs - trunkCG;
...
option.platform_height = 1.0;
option.jump_length = 0.3;
...
%% Contact schedule
contact_phase = round(contactTime/dt);
flight_phase = N-contact_phase; % timesteps spent in the air
% build contact schedule (it needs to be size N+1 because of constraints)
cs = [ones(1,contact_phase) zeros(1,flight_phase) 0];
...
%% Contact Constraints
    if cs(k+1) == 0 % If in flight
        ...
        opti.subject_to(f_r(2,k) <= -5); % Choose some minimum value of normal force to prevent slip
        opti.subject_to(f_r(2,k) >= -300);
        %repeat constraints for rear force
        opti.subject_to(f_r(4,k) <= -5);
...
%s_opts = struct('max_iter',20000, 'linear_solver', 'ma27');
...
if isempty(idx_SL)
    error('Simulink not detected on machine.\nPlease install Simulink (2022b or newer) to run simulation of TO result', 0)
else
    SL_Version = str2double(verStruct(idx_SL).Version);
    SL_Release = verStruct(idx_SL).Release;
    if SL_Version >= 23.2
        out = sim(fullfile('..','SimulinkModels','Hector2D_Simulink','Hector2D_SimulinkModel.slx'), "StopTime", num2str((N-1)*dt));
    elseif isequal(SL_Release, '(R2022b)') || isequal(SL_Release, '(R2023a)')
        out = sim(fullfile('..','SimulinkModels','Hector2D_Simulink','Hector2D_SimulinkModel_2022b.slx'), "StopTime", num2str((N-1)*dt));
        disp("2022b")
    else
        error('Installed Simulink version %s is not currently supported.\nPlease install Simulink 2022b or newer, or contact author for support', SL_Release);
    end
end
...
if INTEGRATION_METHOD == "Euler"
    titles = ["Body X - Euler", "Body Y - Euler", "Body Theta - Euler", "Hip Angle - Euler", "Knee Angle - Euler", "Ankle Angle - Euler"];
end

if INTEGRATION_METHOD == "VI"
    titles = ["Body X - VI", "Body Y - VI", "Body Theta - VI", "Hip Angle - VI", "Knee Angle - VI", "Ankle Angle - VI"];
end
yLabels = ["X (m)", "Y (m)", "Theta (rad)", "q1 (rad)", "q2 (rad)", "q3 (rad)"];

for ii = 1:nx
    figure()
    clf()
    hold on
    plot(out.tout, out.qOut.signals.values(:,ii), '-b', 'LineWidth', 1.5);
...
tSim = out.tout;
qSim = out.qOut.signals.values;

if INTEGRATION_METHOD == "Euler"
    save("EulerForwardResults.mat", "pos","tTO", "qSim", "tSim")
end
...
if INTEGRATION_METHOD == "VI"
    ...
    posVI = pos;
    tTOVI = tTO;
    save("VIForwardResults.mat", "posVI","tTOVI", "qSimVI", "tSimVI")
end
