...
[nx, robotStruct, robotParams] = getCartPendStruct();
...
Kd_j = Kd * diag([0 1]); % Select which joints experience damping
...
%% Terminal Conditions
option.qf = 0; % End in upward position - don't specify track distance
...
%% Dynamics Constraints
    ...
    if INTEGRATION_METHOD == "Euler"
        ...
        % note that pole angle is not actuated
        bk =-Ck - tau_d + [tau_joint(:,k);0];
    ...
    if INTEGRATION_METHOD == "VI"
        ...
        f_d0 = (-tau_d+[tau_joint(:,k);0]);
        f_d1 = (-tau_d+[tau_joint(:,k+1);0]);
    ...
    opti.subject_to(tau_joint(:,k)<=20);
    opti.subject_to(tau_joint(:,k)>=-20);
    opti.subject_to(qk(1) <= 0.5);
    opti.subject_to(qk(1) >= -0.5);
...
%Terminal Conditions
opti.subject_to(q(:,N) == [0; option.qf]); % final position, leave track distance free
...
plot((0:N-1)*dt, 0.1*control(1,:))
hold off
legend(["X_{Cart}", "\theta_{Pole}"], "location", "northwest")
xlabel("Time (s)")
...
% 
% cd(fullfile('..','SimulinkModels','CartPoleSimulink'))
% 
% if isempty(idx_SL)
...
%     if SL_Version >= 23.2
%         out = sim(fullfile('.','CartPoleSimulink.slx'), "StopTime", num2str((N-1)*dt));
%     else
...
% 
% if INTEGRATION_METHOD == "Euler"
%     titles = ["Cart X - Euler", "Pole Angle - Euler"];
% end
% 
% if INTEGRATION_METHOD == "VI"
%     titles = ["Cart X - VI", "Pole Angle - VI"];
% end
