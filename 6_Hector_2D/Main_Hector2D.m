...
Initiate_HECTOR_Simulation; %Initialize Variables for SIMULINK model for Hector
...
[nx, robotStruct, robotParams] = getHECTORStruct2D();
...
for k = 1:N-1
    ...
    pj1k = pHip(qk); % Position of hip joint
    pj2k = pKnee(qk); % Position of knee joint
    pj3k = pAnkle(qk); % Position of ankle joint
...
for k = 1:N
    ...
    joint1Pos(:,k) = pHip(pos(:,k));
    joint2Pos(:,k) = pKnee(pos(:,k));
    joint3Pos(:,k) = pAnkle(pos(:,k));
end
