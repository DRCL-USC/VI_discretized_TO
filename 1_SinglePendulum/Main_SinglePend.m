...
[nx, robotStruct, robotParams] = getSinglePendStruct();
...
%% Initial Conditions 
option.qi = 0; % Start at downward position
option.dqi = 0;  % Start at rest

%% Terminal Conditions
option.qf = pi; % End in upward position
option.dqf = 0; % end at rest 
...
for k=1:N-1
    ...
    J = J + option.alpha_tau * (tau_k')*tau_k; % Add stage cost for joint error and control effort
end
...
%% Dynamics Constraints
    ...
    if INTEGRATION_METHOD == "VI"
        ...
        tau_d = Kd*dq_mp; % Damping is now defined by midpoint velocity

        % Define left and right discrete forces
        f_d0 = (-tau_d+tau_joint(:,k));
        f_d1 = (-tau_d+tau_joint(:,k+1));
...
hold off
legend("\theta", "location", "southwest")
xlabel("Time (s)")
...
cd(fullfile('..','SimulinkModels','SinglePendSimulink'))
...
if isempty(idx_SL)
    ...
    if SL_Version >= 23.2
        out = sim(fullfile('.','SinglePendSimulink.slx'), "StopTime", num2str((N-1)*dt));
    else
