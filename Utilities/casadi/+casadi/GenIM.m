classdef  GenIM < casadi.GenericMatrixCommon & casadi.SparsityInterfaceCommon
  ...
  methods
    ...
    %
    %
      [varargout{1:nargout}] = casadiMEX(248, self, varargin{:});
    end
  ...
  methods(Static)
    ...
    %  std::vector< casadi::Matrix< long long >,std::allocator< casadi::Matrix< long long > > > = SYM(char name, Sparsity sp, int p)
    %  std::vector< casadi::Matrix< long long >,std::allocator< casadi::Matrix< long long > > > = SYM(char name, int nrow, int ncol, int p)
